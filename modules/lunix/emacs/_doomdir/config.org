...
This is a [[https://en.wikipedia.org/wiki/Literate_programming][literate]] Doom Emacs configuration
...
** Tabout
Repurpose =TAB= in insert mode to navigate bracket pairs dynamically
...
** EAF
Manage through nix! Lol

*** Install package
#+begin_src emacs-lisp :tangle packages.el
;; The four EAF apps are sourced from the Nix-side composite built in
;; ../eaf.nix (which itself reads ../_eaf/) — the framework's `eaf.el`
;; copies their elisp into `eaf-<version>/app/` at build time, so they
;; load via `(require 'eaf-browser)` etc. Listing them as `package!`
;; keeps Doom's package-lockfile aware of them.
(package! eaf-browser)
...
*** Vendored EAF, not nixpkgs
#+begin_quote
The =eaf= Emacs package is /not/ pulled from nixpkgs — nixpkgs' copies run
~6–10 months behind upstream. Source is vendored at =../_eaf/= (one
file per app) and fed from =flake=false= flake inputs declared in
=../eaf.nix=. Update loop:
#+end_quote

- =nix flake update eaf= (or =eaf-browser=, =eaf-pdf-viewer=,
  =eaf-file-manager=, =eaf-image-viewer=) bumps the lock entry. The
  version string is derived from the input's =lastModifiedDate= (see
  =../_eaf/version.nix=); =melpaBuild= matches =^.*-unstable-YYYY-MM-DD$=
  and turns it into =melpaVersion= for the LISPDIR, so the format is
  load-bearing.
- Three inputs need an =npmDepsHash= in =../_eaf/npm-hashes.nix= that
  =nix flake update= does /not/ refresh (=browser=, =file-manager=,
  =image-viewer=; =pdf-viewer= has no =package.json=). After bumping,
  either run =nix run nixpkgs#prefetch-npm-deps -- <input-store>/package-lock.json=
  and paste the printed =sha256-...= into the file, or run =nix build .#eaf=
  and copy the expected hash out of the deterministic-output failure.
- A few non-obvious bits live in the Nix side:
  - =aria2= is passed in explicitly to =eaf-browser=. Inside the emacs
    scope the name resolves to the *elisp* aria2 package; nixpkgs does
    the same re-injection in =manual-packages.nix=.
  - The framework's =postPatch= patches =eaf.el= with =--replace-fail= on
    five exact strings. If upstream refactors =eaf.el=, the build aborts
    with the missing string named — that is the intended loud failure;
    re-read upstream and update the patterns.
  - =../_eaf/overlay.nix= patches =plum-py= via =pythonPackagesExtensions=
    (not a top-level =python3Packages= rebind — that one misses
    =python3.pkgs=, which is what =withPackages= actually reads). It
    lives in the *home-manager* scope because this host uses
    =home-manager.useGlobalPkgs = false=, so a NixOS-level
    =nixpkgs.overlays= would never be seen by doom's emacs.

Fast iteration target for the above loop without paying for a full
home-manager rebuild: =nix build .#eaf=. Same overlay, system emacs,
plum-py patched — only good for settling npm hashes and catching
