...
Module activation declarations all tangle to =init.el= (one per
category, below). Package declarations carry =:tangle packages.el=, and
every other block tangles to =config.el=.
...
** term
Eshell only inside the doom =:term= block. Mistty handles the in-Emacs
terminal (see the Mistty section below). Plain =term= left disabled.
...
=(eval +overlay)= is present but =SPC e= was reassigned to Denote.
...
** config
Literate config (=config.org=) with default Doom bindings and
smartparens.
...
** Completion
Doom's =vertico= module (see init.el:30) is the sole completion UI.
** Buffer Behavior
Auto-revert keeps buffers in sync with changes (=global-auto-revert-mode=
also covers non-file buffers like Dired). The =defadvice= prompts
for which buffer to show whenever a window is split, instead of
duplicating the current one. The =flyspell= tweak stops it from
...
** Tabout
Repurpose =TAB= in insert mode: inside an Org / Markdown table it
cycles to the next cell; in bare-text modes (=python=, =yaml=,
=ruby=, =haskell=, =lua=, web modes) it indents; everywhere else it
jumps out of the current pair of delimiters (=sp-up-sexp=). This
replaces Doom's default =electric-indent-mode= binding for TAB in
non-pair contexts so it actually does something.

#+begin_src emacs-lisp
(defun my/tabout ()
  "Tabout-style TAB: in tables, jump to the next cell; otherwise
jump out of the enclosing pair via smartparens, or fall back to
indenting when there is no pair to escape."
  (interactive)
  (cond
   ...
   ((derived-mode-p 'markdown-mode)
    (when (org-at-table-p)
      (call-interactively #'org-table-next-field)))
   ((derived-mode-p 'python-mode yaml-mode ruby-mode
                    haskell-mode lua-mode
                    web-mode css-mode javascript-mode typescript-mode
                    json-mode nxml-mode)
...
(map! :leader
      (:desc "Toggle dirvish" "o /" #'dirvish
       :desc "Mistty toggle"  "v t" #'mistty-toggle
       :desc "Mistty new"     "v n" #'mistty-new
       :desc "Mistty clear"   "v c" #'mistty-clear
       :desc "Mistty reset"   "v r" #'mistty-reset
       :desc "Send region"    "v s" #'mistty-send-region
       :desc "Send line"      "v l" #'mistty-send-line))
#+end_src
...
See =snippets-guide.org= for the format.
...
* Mistty
Mistty is an in-Emacs terminal that uses Emacs' built-in =comint=
(/"emulator"/ mode) instead of libvterm. No C dependency; uses bash as
the shell. Replaces vterm (which has been removed from the =:term=
block above).
...
:END:
Straight from tecosaur: turn on Doom's =+org-prettprose with monospace code), scale headings so the outline is easy to
e outline is easy to
...
Hiding in org-modern will annoy me.
#+begin_src emacs-lisp :tangle packages.el
(package! org-fragtog)
#+end_src
Add hook
...
task. They target files under your =org-directory=. The capture key
=SPC n= (=+org-capture/navigation=) opens the capture menu.
...
Polymode + my-poly-org (vendored from poly-org.el)
#+begin_src emacs-lisp :tangle packages.el
(package! polymode)
#+end_src
...
Vendored from upstream =poly-org.el= (Vitalie Spinu, MIT) under our
own name so we can wire parinfer (and any future =emacs-lisp-mode-hook=
consumer) into the emacs-lisp src-block inner buffer.  Polymode uses
=delay-mode-hooks=-style deferral when cloning inner buffers, so the
plain =emacs-lisp-mode-hook= never fires inside Org src blocks; the
hook is invoked explicitly from a =switch-buffer-functions= slot.
...
(use-package denote
  ...
  ;; Integrate Denote with Doom's org module (resolves duplicate #+title
  ;; when using `+org-tidy-properties` or similar org-module behaviour).
  (after! denote-org (require 'denote-org-extras)))
#+end_src
...
(use-package denote-menu
  ...
  :config
  (setq denote-menu-heading-format 'two-lines))
...
(after! doom
  (map! :leader
        (:prefix ("e" . "Notes (denote)")
         ...
         :desc "menu (panel)"       "m" #'list-denotes
         ...
         (:prefix ("S" . "Silos")
          :desc "create"      "c" #'denote-silo-create-note
          :desc "open/create" "o" #'denote-silo-open-or-create
          :desc "select"      "s" #'denote-silo-select-silo-then-command
...
~org-html--format-toc-headline~. Since we can actually accomplish
the desired effect by adding advice /around/ the function, without
...
Since we have =verbatim= and ~code~, let's make use of the difference. We can use ~code~ exclusively for code snippets and commands like: src_elisp{(message "Hello")} in batch-mode Emacs prints to stdout. Then we can use =verbatim= for miscellaneous 'other monospace' like keyboard shortcuts: "either =C-c C-c= or =C-g= is likely the most useful keybinding in Emacs", or file names: "I keep my configuration in =~/.config/doom/=", among other things.
