...
(doom! :editor
       ...
       (whitespace +guess +trim))  ; a butler       ;;word-wrap         ; soft wrapping with language-aware indent)
nguage-aware indent)
#+end_src
...
** term
Eshell and Vterm. Plain =term= left disabled — eshell handles most.
...
strategies (visible text, other buffers, filenasymbols) in turn, which is more capable than the default =dabbrev=.
e default =dabbrev=.
...
** Buffer Behavior
Auto-revert keeps buffers in sync with changes madnon-file buffers like Dired). The advice prompts for which buffer to
 for which buffer to
...
(defun my/tabout ()
  ...
  (cond
   ((and (derived-mode-p 'org-mode)
         ...
         (org-at-table-p))
    (call-interac      ((and (derived-mode-p 'markdown-mode)
de-p 'markdown-mode)
...
#+begin_src emac        :desc "Toggle dirvish"          "o /"   #'dirvish
...
** Markdown
*** Titles
#+begin_src emacs-lis'(markdown-header-delimiter-face :foreground "#616161" :height 0.9)
...
(after! org
  (setq org-modern-star 'replace                 ; swap heading asterisks for bullets
        org-modern-list '((?- . "•") (?+ . "◦") (?* . "▪"))
        org-modern-block-fringe 3             ; keep blocks aligned with text
        org-modern-table t
...
task. They target files under your =org-directory=n=) opens the capture menu.
ns the capture menu.
...
(after! ox-html
  ...
  (advice-add 'org-html-property-drawer :around #'org-htm)
ock-collapsable)
  )
#+end_src
...
To add this, we need to change one line in [[file:lisp/org/lisp/ox-html.el::(format "<a href=\"#%s\">%s</a>"][org-html--forSince we can actually accomplish the desired effect by adding advice /around/ the
...
line's usual output,
  to allow the TOC to be a    :around #'org-html--format-toc-headline
    (if (or (not org-fancy-html-export-mode) (bound-and-true-p org-msg-export-in-progress))
...
iSince we have =verbatim= and ~code~, let's make use of the difference.
eWe can use ~code~ exclusively for code snippets and commands like: "calling
mands like: "calling
src_elisp{(message "Hello")} in batch-mode Emacs prints to Then we can use =verbatim= for miscellaneous 'other monospace' like keyboard
